angular.module('App')
  .controller('ProduitsParMarqueCtrl', function ($scope,AuthFactory,FavorisFactory,baseUrl,PanierFactory,PanierBWMFactory,FavorisFactory,$http,$interval,$location,$anchorScroll,$rootScope,$location,$timeout,$routeParams) {
    ...
    if (getUrlVars()["marque"]!= undefined ){
        $scope.marque=getUrlVars()["marque"];
    }
    else{
     $scope.marque="null"; 
    }
    ...
    $http.get(baseUrl+"product/model/marque/"+$scope.marque+"/"
      +$scope.currentPage+"/"+$scope.filterOption+"/"+$scope.filterMinPrice+"/"
      +$scope.filterMaxPrice).then(function (response) {
    ...
    $rootScope.slider = {
        ...
        options: {
          ...
          onEnd: function(sliderId, modelValue, highValue, pointerType){
            ...
            window.location ="marque.php?marque="+$scope.marque+"&filter="+$scope.filterOption+"&minprice="+$scope.filterMinPrice+"&maxprice="+$scope.filterMaxPrice;
                ...
                if($scope.statut=="200"){
                  ...
                  $timeout(function () {
                   ...
                   if($routeParams["minprice"]=="null"){
                    ...
                    $rootScope.slider.minValue=parseInt($routeParams["minprice"]);
                   }

                    if($routeParams["maxprice"]=="null"){
                      $rootScope.slider.maxValue=parseInt(response.data.data.plusGrandPrix);
                    }else{
                      $rootScope.slider.maxValue=parseInt($routeParams["maxprice"]);
                    }
