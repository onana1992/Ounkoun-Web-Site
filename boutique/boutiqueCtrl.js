angular.module('App')
  .controller('BoutiqueCtrl', function ($scope,AuthFactory,FavorisFactory,baseUrl,PanierFactory,PanierBWMFactory,FavorisFactory,$http,$interval,$location,$anchorScroll,$rootScope,$location,$timeout,$routeParams) {
    ...
    $rootScope.slider = {
        ...
        options: {
          ...
          onEnd: function(sliderId, modelValue, highValue, pointerType){
            ...
            window.location = "boutique.php?nom="+$scope.boutique+"&filter="+$scope.filterOption+"&minprice="+$scope.filterMinPrice+"&maxprice="+$scope.filterMaxPrice+"&marque="+$scope.filterMarque;
          }
        }
    };
    ...
    $scope.init = function () {
      ...
      $http.get(baseUrl+"product/model/seller/"+$scope.boutique+"/"+$scope.currentPage+"/"+$scope.filterOption+"/"+$scope.filterMinPrice+"/"+$scope.filterMaxPrice+"/"+$scope.filterMarque).then(function (response) {
      $scope.statut= response.data.statut;

       
      $scope.products= response.data.data.products;
      $scope.extractedMarque = response.data.data.marques;
      $scope.extractedCategories= response.data.data.categories;
