angular.module('App')
  .controller('SousCategoriesCtrl', function ($scope,AuthFactory,baseUrl,PanierFactory,PanierBWMFactory,FavorisFactory,$http,$interval,$location,$anchorScroll,$rootScope,$location,$timeout,$routeParams) {
    ...
    if (getUrlVars()["scat"]!= undefined ){
      $scope.scategorie= getUrlVars()["scat"];
    }

    if (getUrlVars()["sscat"]!= undefined ){
      $scope.sscategorie= getUrlVars()["sscat"];
    }
    else{
      $scope.sscategorie= "toute les categories";
    }
    ...
    $rootScope.slider = {
        ...
        options: {
          ...
          onEnd: function(sliderId, modelValue, highValue, pointerType){
            ...
            window.location ="sous-categorie.php?cat="+$scope.categorie+"&scat="+$scope.scategorie+"&filter="+$scope.filterOption+"&minprice="+$scope.filterMinPrice+"&maxprice="+$scope.filterMaxPrice+"&marque="+$scope.filterMarque;
         }
        }
    };
    ...
    $scope.init = function () {
         $scope.isLoaded=false;
      // extraction des sous-categories
      $http.get(baseUrl+"category/sscategory/"+$scope.categorie+"/"+$scope.scategorie).then(function (response) {
        $scope.statut= response.data.statut;
        $scope.sscategories= response.data.data;
        }, function () {
          // alert("not ok 11");
          }).then(function () {
      });
      ...
      if ($scope.sscategorie=="toute les categories"){
        
        $http.get(baseUrl+"product/model/scategory/"+$scope.scategorie+"/"
          +$scope.currentPage+"/"+$scope.filterOption+"/"+$scope.filterMinPrice+"/"
          +$scope.filterMaxPrice+"/"+$scope.filterMarque).then(function (response) {
          $scope.statut= response.data.statut;
          $scope.marques = response.data.data.marque;
        ...
        $http.get(baseUrl+"product/model/sscategory/"+$scope.sscategorie+"/"
        + $scope.currentPage+"/"+$scope.filterOption+"/"+$scope.filterMinPrice+"/"
        +$scope.filterMaxPrice+"/"+$scope.filterMarque).then(function (response) {
        $scope.statut= response.data.statut;
        $scope.marques = response.data.data.marque;
    ...
    $scope.chooseMarque = function (marque) {
      ...
      $location.path("/sous-categories/"+ $routeParams["cat"]+"/"+$routeParams["scat"]+"/"+$routeParams["sscat"]+"/"+$scope.selectedPage+"/"+$routeParams["filter"]+"/"+$routeParams["minprice"]+"/"+$routeParams["maxprice"]+"/"+$scope.filterMarque);

    };

    $scope.chooseScategorie = function (ssCat) {
      $scope.sscategorie= "toute les categories";
      $scope.currentPage=1;
      $scope.selectedPage="1";
      $location.path("/sous-categories/"+ $routeParams["cat"]+"/"+$routeParams["scat"]+"/"+ssCat+"/"+$scope.selectedPage+"/"+$routeParams["filter"]+"/"+$routeParams["minprice"]+"/"+$routeParams["maxprice"]
        +"/"+ $routeParams["marque"]);
    };
  ...
  $scope.sendComment =function(){
    ...
    var myobject = {'idModel':$scope.selectedProduct.id,
                    'login': userLogin.login,'name':userLogin.name,
                    'firstName':userLogin.firstName,
                    'valeur':$scope.commentaire
    };
