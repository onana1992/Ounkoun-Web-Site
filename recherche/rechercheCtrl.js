angular.module('App')
  .controller('RechercheCtrl', function ($scope,AuthFactory,FavorisFactory,baseUrl,PanierFactory,PanierBWMFactory,FavorisFactory,$http,$interval,$location,$anchorScroll,$rootScope,$location,$timeout,$routeParams) {
    ...
    if (getUrlVars()["cat"]!= undefined ){
      $scope.searchText= getUrlVars()["text"];
    }else{
    ...
    $rootScope.slider ={
        ...
        options: {
          ...
          onEnd: function(sliderId, modelValue, highValue, pointerType){
            ...
            window.location = "recherche.php?text="+$scope.searchText+"&cat="+$scope.categorie+"&filter="+$scope.filterOption+"&minprice="+$scope.filterMinPrice+"&maxprice="+$scope.filterMaxPrice+"&marque="+$scope.filterMarque;
         }
        }
    };
    ...
    $scope.init = function() {
      ...
      $scope.isLoading=true;
      $http.get(baseUrl+"product/model/search/"+$scope.searchText+"/"
        +$scope.currentPage+"/"+$scope.filterOption+"/"+$scope.filterMinPrice+"/"
        +$scope.filterMaxPrice+"/"+$scope.filterMarque+"/"+$scope.categorie).then(function (response) {
        ...
        $scope.marques[0]=response.data.data.marques[0].name;
       // for(var i=0; i<$scope.searchMarques.length; i++){
          //if($scope.marques.indexOf($scope.searchMarques[i].name)== -1 ){
           // $//scopemarques.push($scope.searchMarques[i].name);
          //}
       // }
